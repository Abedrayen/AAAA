...
import {  getTopYellowCards } from "../../API/API"
...
const TopYellowCards=()=>{
    ...
    useEffect(() => {
        const getStas = async () => {
          ...
          try {
            ...
            const response = await getTopYellowCards()
            ...
            if (response) {
              ...
              setTopYellowCards(formattedResponse);  // Correctly setting the top scorers
            }
    ...
    return (
        ...
        <h1 className="text-4xl font-bold text-[#1F5059]">{t('topyellowcards')}</h1>
        ...
        <table className="z-30 table-auto border-collapse text-left">
          ...
          <tbody>
            {topYellowCards.map((item:any, index) => (
              <tr
                key={item.player.id}
                className={`border-b ${
